  ...
  try {
    ...
    const regRes = await fetch(`${serverUrl}/api/agents`, {
      ...
      body: JSON.stringify({
        ...
        // No title: a generated one would overwrite whatever the manifest
        // already calls this agent. The server keeps the existing name.
      }),
    });

    if (!regRes.ok) {
      // A refused lane is the most likely rejection and the most actionable,
      // so say which lane it collided with rather than only a status code.
      const detail = await regRes.json().catch(() => null);
      if (detail?.error) {
        console.error(`
❌ ${detail.error}`);
        for (const c of detail.conflicts ?? []) console.error(`   ${c}`);
        console.error(`
Pick a lane nobody owns, or ask the architect which one is yours.`);
        process.exitCode = 1;
        return;
      }
