...
import { globsOverlap } from "./lanes.mjs";
    ...
    if (url.pathname === "/api/agents" && req.method === "POST") {
      ...
      req.on("end", () => {
        try {
          ...
          // Merge, never clobber. A re-join used to overwrite the entry with
          // defaults, so a hand-authored manifest lost its branch, title,
          // colour, icon and worktree every time its agent reconnected — which
          // made the manifest impossible to curate.
          const existing = orchestrator.manifest.agents[cleanName] ?? {};
          const provided = JSON.parse(body || "{}");
          const newAgentObj = {
            ...existing,
            role,
            branch: branch || existing.branch || `feat/${cleanName}`,
            owns: laneArray.length ? laneArray : existing.owns ?? [`src/${cleanName}/**`],
            title: provided.title || existing.title || `${name} Agent`,
            color: provided.color || existing.color || "blue",
            icon: provided.icon || existing.icon || "🤖",
          };

          // Self-registration must not be able to break the one invariant the
          // whole tool rests on. An agent that claims a lane another agent
          // already owns is exactly the collision this project exists to
          // prevent, and accepting it silently wrote the overlap to disk.
          const clash = [];
          for (const [other, a] of Object.entries(orchestrator.manifest.agents)) {
            if (other === cleanName) continue;
            for (const mine of newAgentObj.owns) {
              for (const theirs of a.owns ?? []) {
                if (globsOverlap(mine, theirs)) clash.push(`"${mine}" overlaps ${other}'s "${theirs}"`);
              }
            }
          }
          if (clash.length) {
            ...
            res.end(JSON.stringify({
              error: "Lane overlaps an agent already registered. Lanes must be disjoint.",
              conflicts: clash,
            }));
            return;
          }
