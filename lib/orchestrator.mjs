...
export class Orchestrator extends EventEmitter {
  ...
  /**
   * Inject a message from a named agent.
   *
   * Without this every agent posting through the documented HTTP API was
   * recorded as the human supervisor, so a multi-agent log could not say who
   * said anything. Role comes from the manifest, never from the caller, so an
   * agent cannot promote itself by claiming one.
   */
  injectAgentMessage(sender, content, target = null) {
    const known = (this.manifest.agents || {})[sender];
    if (!known) return this.injectHumanMessage(content, target);
    return this.bus.publish({
      sender,
      receiver: target || "all",
      role: known.role || ROLES.BUILDER,
